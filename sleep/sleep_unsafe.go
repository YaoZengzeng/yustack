...
// notifications (waker). It offers O(1) complexity, which is different from
// multi-channel selects which have O(n) complexity (where n is then number of
// channels) and a considerable constant factor
//
// It is similar to edge-triggered epoll waits, where the user registers each
// object of interest one, and then repeatedly wait on all of them
//
...
//
//	The notification are edge-triggered, which means that if a Waker calls
...
//
//	The "unsafeness" here is the casts to/from unsafe.Pointer, which is safe
...
//
// None of the methods in a Sleeper can called concurrently. Wakers that have
// been added to a sleeper A can only be added to another sleeper after A.Done()
// returns. These restrications allow this to be implemented lock-free
//
// This struct is thread-compatible
type Sleeper struct {
	// sharedList is a "stack" of asserted wakers. They atomatically add
	...
	// waiter, and thus doesn't have to be accessed atomatically. When
	// fetching more wakers, the waiter will first go through this list, and
	// only when it's empty will it atomatically fetch wakers from sharedList
	localList 	*Waker
...
// nextWaker returns the next waker in the notification list, blocking if needed
func (s *Sleeper) nextWaker(block bool) *Waker {
	...
	if s.localList == nil {
		for atomic.LoadPointer(&s.sharedList) == nil {
			// Fail request if caller requested that we don't block
			if !block {
				return nil
			}

			// Indicate the wakers that we're about to sleep,
			// this allows them to abort the wait by setting
			// waitingG back to zero (which we'll notice
			// before commiting the sleep)
...
// available, it is required right away. Otherwise, the behaviour depends on the
...
// the waker; then  'ok' is false, 'id' is undefined
//
...
func (s *Sleeper) Done() {
	...
	// asserted yet. By atomically sitching w.s to nil, we guarantee that
...
// enqueAssertedWaker enqueues an asserted waker to the "ready" cicular list
...
func (s *Sleeper) enqueueAssertedWaker(w *Waker) {
	...
	for {
		...
		if atomic.CompareAndSwapUintptr(&s.waitingG, g, 0) {
			if g != preparingG {
				// We manage to get a G, Wake it up
				goready(g, 0)
			}
		}
	}
...
type Waker struct {
	...
	//		given sleeper. Once is transitions to asserted state, the
