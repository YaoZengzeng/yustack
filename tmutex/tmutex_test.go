...
func TestBasicLock(t *testing.T) {
	...
	// be locked because the mutex is held
...
func TestTryLock(t *testing.T) {
	...
	// Try to lock. It should succeeded
	if !m.TryLock() {
		t.Fatalf("TryLock failed on unlocked mutex")
	}

	// Try to lock again, it should fail now
	if m.TryLock() {
		t.Fatalf("TryLock succeeded on locked mutex")
	}

	// Try blocking lock the mutex from a different goroutine. This must be
	// blocked because the mutex is held
	ch := make(chan struct{}, 1)
	go func() {
		m.Lock()
		ch <-struct{}{}
...
func TestMutualExclusion(t *testing.T) {
	...
	// have each one increment a counter "iters" times within the ciritical
	// section established by the mutex
	//
	// If at the end the counter is gr * iters, then we know that goroutines
	// can concurrently within the critical section
	//
...
func TestMutualExclusionWithTryLock(t *testing.T) {
	...
	// Similar to the previous, with the addition of some goroutines that only
	// increment the counter if TryLock succeeds
	const gr = 1000
	const iters = 100000
